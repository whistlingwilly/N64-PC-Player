...
---
First Time Setup
Plug in your DreamDump64
Launch the app via `run.bat`
Go to Help → Install Emulator to download Mupen64Plus (~8MB)
Insert a cartridge — the app detects and reads it automatically
Click PLAY
---
Controller Setup
Go to Emulator → Controller Setup:
Select USB / Gamepad or Keyboard
Pick your controller from the dropdown
Click any N64 button slot, then press the matching button on your controller
Or use the N64 adapter, Xbox, or PlayStation preset to auto-fill
Click Save & Apply
---
Project Structure
```
...
CRC32 — exact match against No-Intro verified database (confirms authentic dump)
Game code — 4-character code from ROM header (e.g. `NSME` = Super Mario 64 USA)
Title string — 20-character title from ROM header as fallback
Launching
...
---
Acknowledgements
Mupen64Plus — open source N64 emulator
libretro thumbnails — cover art CDN
No-Intro — verified ROM database
Inspired by the Epilogue Operator
